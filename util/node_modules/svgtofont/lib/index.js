...
export default async (options = {}) => {
    ...
    try {
        ...
        const multicolorGlyphNames = new Set();
        ...
        Object.keys(unicodeObject).forEach((name, index, self) => {
            ...
            if (multicolorGlyphNames.has(name))
                return;
            if (!infoData[name])
                infoData[name] = {};
      ...
      `);
        ...
        if (options.multicolor) {
            for (const [iconName, mcInfo] of Object.entries(multicolorMap)) {
                ...
                const spans = mcInfo.layers.map((_, i) => `<span class="path${i + 1}"></span>`).join('');
                cssIconHtml.push(`<li class="class-icon"><i class="${symbolName}">${spans}</i><p class="name">${iconName}</p></li>`);
                // HTML for unicode preview (show each layer separately)
                mcInfo.layers.forEach((layer, idx) => {
                    unicodeHtml.push(`<li class="unicode-icon"><span class="iconfont">${layer.unicode}</span><h4>${iconName}-path${idx + 1}</h4><span class="unicode">&amp;#${layer.encodedCode};</span></li>`);
                });
            }
        }
        if (options.useCSSVars) {
