...
export function resolveEffectiveFill($, $el) {
    ...
    while ($parent.length > 0) {
        const tagName = $parent[0]?.tagName || $parent[0]?.name;
        if (tagName === 'svg')
            break;
...
// --- Analyze SVG colors ---
export function analyzeSvgColors(svgContent) {
    ...
    const colorGroups = Array.from(colorMap.entries()).map(([color, elements]) => ({
        color,
        elements,
    }));
    return {
...
function serializeElementWithTransforms($, $el) {
    const transforms = [];
...
// --- Generate layer SVGs ---
export function generateLayerSvgs(analysis) {
...
//# sourceMappingURL=multicolor.js.map